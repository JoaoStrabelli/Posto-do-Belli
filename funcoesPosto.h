...
void adicionarCarro(int totalCarros, int *carrosFila, float tanque, struct TCarro espera[]){
...
void abastecimento(float *qntComb, int *carrosFila, int qntAbastecida, float valorTot, 
...
struct TCarro atend[]){
    ...
    if(*carrosFila > 0){
        ...
        if(*qntComb != 0){
            ...
            while(!scanf("%d", &qntAbastecida) || (qntAbastecida <= 0) || (qntAbastecida > *qntComb)){
